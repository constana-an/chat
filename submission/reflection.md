...
The hardest part was understanding what "mute" really does. At first, I thought mute may stop notifications completely. But the as-built diagram shows routeMessage() decides inbox by message type, and muted is not used to decide inbox or alert. It only records information like bypassedMute and the reason. Also, unreadFor() always runs. So muted messages still count as unread, and mentions still enter the inbox and can alert. I am still not sure if the 100-notification limit and no SSE delivery guarantee are intentional.
